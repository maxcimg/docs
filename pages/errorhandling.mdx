...
---
<AccordionGroup>
    <Accrodion title="Device is not connecting" defaultOpen={false}>
    ...
    <Accrodion title="My board is not in the list" defaultOpen={false}>
    ...
    <Accrodion title="Devboard connected, but bus connection failed"  defaultOpen={false}>
    ...
    <Accrodion title="Device address is not found"  defaultOpen={false}>
      * Verify the bus address in your configuration settings
    </Accrodion>

    <Accrodion title="Device connected, but no data is being received"  defaultOpen={false}>
      * Verify that your Setup is configured correctly

      * Check that the board and component are properly wired
    </Accrodion>

    <Accrodion title="System failed to read registers"  defaultOpen={false} />

    <Accrodion title="&#x22;Generate testing&#x22; button is not working" icon="bolt" defaultOpen={false}>
      * Confirm that you have an active paid subscription to access testing features

      * Check for any errors in the Component Model
    </Accrodion>

    <Accrodion title="Error: ”There was a problem with the fetch operation”"  defaultOpen={false}>
